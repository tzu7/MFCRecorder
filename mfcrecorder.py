...
if __name__ == '__main__':
    try:
        if not os.path.isfile('/config/config.conf'):
            shutil.copy2('config/config.conf', '/config/config.conf')
        if not os.path.isfile('/config/wanted.json'):
            shutil.copy2('config/wanted.json', '/config/wanted.json')
        config = classes.config.Config(os.path.join('/config', 'config.conf'))
    except:
        config = classes.config.Config(os.path.join('config', 'config.conf'))
